...
export const route = (app: Application) => {
    ...
    app.post("/locality/", 
            ...
            validator.body("idProvince").notEmpty().withMessage(ErrorCode.FIELD_REQUIRED),
            run(async (req: Request, resp: Response) =>{
                const errors = validator.validationResult(req)
                if(errors && !errors.isEmpty()){
                    throw ValidatorUtils.toArgumentsException(errors.array())
    ...
    app.put("/locality/",
        validator.query("id").notEmpty().withMessage(ErrorCode.FIELD_REQUIRED),
            ...
            const localityOriginal : LocalityDto = await  service.instance.findLocalityById(id)
			if (!dto["nameLocality"]) {
                 dto["nameLocality"] = localityOriginal["nameLocality"];
            }
            if (!dto["idProvince"]) {
                 dto["idProvince"] = localityOriginal["idProvince"];
            }
            const locality = await service.instance.updateLocality(id,{
                    nameLocality: dto["nameLocality"],
                    idProvince: dto["idProvince"]
            })
            resp.json(locality) 
