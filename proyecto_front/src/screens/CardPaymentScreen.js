...
export const CardPaymentScreen = ({route, navigation}) => {
    ...
    const makePayment = async () => {
        const cardNumberError = roomVerificator(cardNumber.value)
        const expirationDateError = roomVerificator(expirationDate.value)
        const cCVError = roomVerificator(cCV.value)
        const nameError = roomVerificator(name.value)
        console.log(order)
    ...
    const email = {
        ...
        inicio: String(order.start.toLocaleDateString() + " desde la hora " + order.start.toLocaleTimeString()) ,
        //mail del dueño
        duenoSala: String(
            sala.idOwner
            //order.sala.email
    ...
    return (
        <StateScreen >
            <Screen navigation={navigation} >
                ...
                />
                <TextInput label = "Tarjeta de credito" 
                onChangeText = {(text) => setCardNumber({ value: text, error: '' })}
                error={!!cardNumber.error}
                errorText={cardNumber.error}
                />
                <TextInput label = "Vencimiento" 
                onChangeText = {(text) => setExpirationDate({ value: text, error: '' })}
