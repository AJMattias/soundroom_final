...
export  function AdminComision({navigation}){
    ...
    const actualizarComision = async () =>{
      ...
      const comisionActualizada = await comisionesService.actualizarComision(comisionSelected.id, comisionSelected.value)
      console.log(comisionActualizada)
      replace()
    }

    const listComisiones =  comisionesToPick.map((comision) => 
    <Picker.Item key ={comision.id} label={comision.label} value={comision.value}
    />)
